...
// an object to hold lovebirds
const lovebirdParticlesStartCount = 0
let lovebirdFlock = new LovebirdFlock()
...
// Spring things
let springSystem = new SpringSystem()
...
document.addEventListener("DOMContentLoaded", function(){
	...
	lightmap = new Lightmap({
		...
		drawChannels: function() {
			...
			lovebirdFlock.lovebirds.forEach(boid => lightmap.drawBlurryLight({
				pt: boid.position, 
				channels: [255, 0, 0], 
				intensity: 1,
				size: 1
	...
	mainP5 = new p5(
		...
		function(p) {
			...
			p.setup = () => {
				...
				createSlider({label:"friendliness", min:0, max: 200, defaultValue: 50})
				...
				// createSlider({label:"forceDisplay", min:.1, max: 4, defaultValue: .4, step: .1})
				// createSlider({label:"lovebirdCohesion", min:0, max: 200, defaultValue: 50})
				// createSlider({label:"lovebirdAlignment", min:0, max: 200, defaultValue: 50})
				// createSlider({label:"lovebirdWander", min:0, max: 200, defaultValue: 50})
				...
				createSlider({label:"Drag", min:.001, max: 0.8, defaultValue: .014, step: .001})
				// createSlider({label:"katesBorder", min:1, max: 100, defaultValue: 30, step: 1})

				createSlider({label:"windForce", min:0, max: 10, defaultValue: 5, step: 1})
				...
				if (p.canvas.parentNode.querySelector(":hover") == p.canvas) {
					...
					switch(drawMode) {
						...
						case "spring": 
							...
							break;
						case "lovebird": 
							lovebirdFlock.addLovebird(mousePos)
							break;
						// case "rocket": 
			...
			p.draw = () => {
				...
				// Draw lovebirds
				lovebirdFlock.draw(p)
				if (FLAGS.drawLovebirdDebug) {
					lovebirdFlock.debugDraw(p)
				}
