...
let app = {
	...
	populate() {
		...
		for (var i = 0; i < app.popCount; i++) {
			...
			let x = Math.random() * canvasW*.5 + (canvasW - 100)*.9*(pct - .5) - 0.8
			let y = Math.random() * canvasH*(.8 + .1*(i%2)) - 1
		...
		mounted() {
			let p = new p5((p) => {
				...
				p.setup = () => {
					...
					let cvs = p.createCanvas(0.75*canvasW, canvasH)
					cvs.position(0.25*canvasW, 0, 'fixed')
